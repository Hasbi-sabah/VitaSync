...
def get_all_hcw_appointments(hcwId, current_user):
    ...
    if data:
        ...
        res = [appointment.to_dict() for appointment in database.appt_lookup(start_time, end_time, hcwId=str(hcwId))]
    else:
        # If no parameters are provided, retrieve all non-archived appointments for the hcw
        res = [appointment.to_dict() for appointment in hcw.appointments if not appointment.archived]
