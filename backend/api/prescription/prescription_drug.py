...
def get_all_prescription_drugs(prescriptionId, current_user):
    ...
    if current_user.role == 'prescription' and current_user.profileId != prescription.prescribedForId:
...
def add_prescription_drug(prescriptionId, current_user):
    ...
    if database.search(DrugPrescribed, prescriptionId=str(prescriptionId), drugId=drug.id):
    ...
    return jsonify(database.get_by_id(DrugPrescribed, str(drug_prescribed.id)).to_dict())
